...
import { saveTicketSamplingAction, deleteTicketSamplingAction, importTicketSamplingAction } from '@/app/actions/checklists';

interface TicketSamplingTableProps {
  data: any[];
  isQA: boolean;
  petugasList: string[];
}
...
export function TicketSamplingTable({ data, isQA, petugasList }: TicketSamplingTableProps) {
    ...
    reader.onload = (event) => {
      try {
        ...
        const mappedRows = json.map(row => {
          ...
          let rawCat = catKey ? String(row[catKey]).trim() : 'Chat';
          if (rawCat) {
            rawCat = rawCat.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
          }
        ...
        if (confirm(`Apakah Anda yakin ingin meng-import ${mappedRows.length} data list ticket sampling?`)) {
          startTransition(async () => {
            const res = await importTicketSamplingAction(mappedRows);
            if (res.success) {
              alert(res.message);
            } else {
              alert(res.error);
  ...
  const categories = ['Chat', 'Email', 'Call', 'Social Media', 'Walk-in'];
  ...
  const handleToggle = (row: any, month: string) => {
    ...
    startTransition(async () => {
      const res = await saveTicketSamplingAction(updatedRow);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const exists = data.some(
      row => row.petugas_name?.toLowerCase() === formPetugasName.toLowerCase() && 
             ...
             String(row.category).toLowerCase() === formCategory.toLowerCase()
    );
    if (exists) {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `Ticket Sampling ${selectedYear}`);
    XLSX.writeFile(wb, `Ticket_Sampling_Petugas_${selectedYear}.xlsx`);
  };
                ...
                filteredData.map((row) => (
                  <tr key={row.__rowIndex} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className="inline-block px-2 py-0.5 rounded-md bg-blue-50 text-[#1E3A8A] font-bold text-[10px] border border-blue-100">
                        {row.category || 'Chat'}
                      </span>
                    </td>
                    {months.map(m => {
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-800">Tambah Baris Checklist Ticket</h3>
